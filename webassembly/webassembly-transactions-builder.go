...
func createDelegateTx_Float(this js.Value, args []js.Value) interface{} {
	return promiseFunction(func() (interface{}, error) {
		...
		tx, err := app.TransactionsBuilder.CreateDelegateTx_Float(txData.From, txData.Nonce, txData.DelegateAmount, txData.DelegateNewPublicKeyHashGenerate, txData.DelegateNewPubKeyHash, txData.DelegateNewFee, txData.Data, txData.Fee, txData.PropagateTx, txData.AwaitAnswer, false, func(status string) {
			args[1].Invoke(status)
			time.Sleep(10 * time.Millisecond)
		})
		if err != nil {
...
func createUnstakeTx_Float(this js.Value, args []js.Value) interface{} {
	return promiseFunction(func() (interface{}, error) {
		...
		type DelegateTxFloatData struct {
			...
			Data          *wizard.TransactionsWizardData                         `json:"data"`
			Fee           *transactions_builder.TransactionsBuilderFeeFloatExtra `json:"fee"`
			PropagateTx   bool                                                   `json:"propagateTx"`
			AwaitAnswer   bool                                                   `json:"awaitAnswer"`
		}
